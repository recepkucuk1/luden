...
async function downloadHomeworkPDF(hw: HomeworkContent) {
    ...
    <Document title={hw.title} author="LudenLab">
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.badges}>
          ...
          {hw.duration && (
            <Text style={[styles.badge, { backgroundColor: "#f4f4f5", color: "#52525b" }]}>{hw.duration}</Text>
          )}
          {hw.targetArea && (
            <Text style={[styles.badge, { backgroundColor: "#f4f4f5", color: "#52525b" }]}>{hw.targetArea}</Text>
          )}
        </View>

        {hw.introduction && (
          <View style={styles.intro}>
            <Text style={styles.introText}>{hw.introduction}</Text>
          </View>
        )}
        ...
        {hw.materials && hw.materials.length > 0 && (
          <View style={{ marginBottom: 12 }}>
            ...
            {hw.materials.map((m, i) => (
              <Text key={i} style={styles.matItem}>• {m}</Text>
            ))}
          </View>
        )}
        ...
        {hw.frequency && (
          <Text style={styles.freq}>Önerilen Sıklık: {hw.frequency}</Text>
        )}
  ...
  a.href     = url;
  a.download = `${hw.title.replace(/\s+/g, "_")}.pdf`;
  a.click();
